...
dist: dist-clean
	...
	mkdir -p dist/linux/aarch64 && GOOS=linux GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o dist/linux/aarch64/dockerize
	mkdir -p dist/linux/arm64 && GOOS=darwin GOARCH=arm64 go build -ldflags "$(LDFLAGS)" -o dist/darwin/arm64/dockerize
